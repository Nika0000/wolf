...
You can run the quickstart script directly with `curl -sSL https://raw.githubusercontent.com/jbailes/wolf/main/quickstart/wolf.sh | bash` or see the quickstart guide and script at https://github.com/JBailes/wolf/tree/main/quickstart for a fully automated script that will handle setting the environment up for you.
